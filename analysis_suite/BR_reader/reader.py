...
from analysis_suite.BR_reader.parsing import (
        unpack_string,
...
from analysis_suite.BR_reader.errors import (
        BioRadInvalidFileError, BioRadParsingError
        )
from analysis_suite.BR_reader.constants import BLOCK_PTR_TYPES
try:
    import numpy as np
...
class Reader():
    ...
    def get_image(self, invert=False):
        ...
        # PIL interprets mode 'I;16' as "uint16, little-endian"
        img_out = Image.new('I;16', (img_x, img_y))

        if HAS_NUMPY:
            # make sure u16in little-endian, output bytes
            outpil = img_data.astype(img_data.dtype.newbyteorder("<")).tobytes()
        else:
            # little-endian u16 format
            # TODO: is it ok to use *args with huge len(args) ??
            outpil = struct.pack(
